 ...
 * blocked account — this never tells a caller *why* a login failed, only
 * that it did.
 */
class AuthenticateUserAction
{
    ...
    public function handle(string $email, string $password, bool $remember): User
    {
        ...
        // Admin accounts sign in only through /admin (Filament's own
        // canAccessPanel() guard) — never through the public member-facing
        // form. Credentials already matched at this point, so naming the
        // reason isn't an information leak the way it would be above.
        if ($user->roles()->where('slug', 'admin')->exists()) {
            Auth::guard('web')->logout();

            throw ValidationException::withMessages([
                'email' => 'Admin accounts must sign in at /admin.',
            ]);
        }
