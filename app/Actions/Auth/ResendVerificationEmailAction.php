...
/**
 * Deliberately silent about whether $email belongs to any account: the
 * caller always shows the same generic message regardless of what happens
 * in here, so this method never returns anything the controller could
 * accidentally leak through a different response. Only a
 * PendingVerification account is eligible — an unknown email or an
 * already-Active one is a no-op, not an error.
 */
