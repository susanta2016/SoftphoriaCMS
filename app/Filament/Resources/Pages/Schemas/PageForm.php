    ...
    private static function sectionSchema(): array
    {
        return [
            ...
            TextInput::make('content_json.eyebrow')->label('Eyebrow (small label above the heading)')
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::Hero, PageSectionType::Cta, PageSectionType::Gallery, PageSectionType::Testimonials, PageSectionType::Portfolio, PageSectionType::BlogPosts])),
            ...
            Textarea::make('content_json.description')->label('Description')->rows(3)
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::Cta, PageSectionType::Gallery, PageSectionType::Portfolio, PageSectionType::BlogPosts])),
            ...
            TextInput::make('content_json.anchor')->label('Anchor ID')
                ...
                ->regex('/^[a-z0-9-]+$/')
                ->maxLength(60)
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::Gallery, PageSectionType::Cta, PageSectionType::Testimonials, PageSectionType::Portfolio, PageSectionType::BlogPosts])),
            ...
            Select::make('content_json.background')
                ...
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::Gallery, PageSectionType::Portfolio, PageSectionType::BlogPosts])),
            ...
            // Featured Portfolio / Latest Blog Posts: the cards come from
            // their admin modules, never from this section.
            TextInput::make('content_json.limit')
                ->label('Maximum items to show')
                ->numeric()
                ->minValue(1)
                ->maxValue(12)
                ->default(fn (Get $get): int => $get('section_type') === PageSectionType::BlogPosts->value ? 3 : 6)
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::Portfolio, PageSectionType::BlogPosts])),
            ...
            Placeholder::make('blog_posts_notice')
                ->hiddenLabel()
                ->content('Shows the newest published posts from Blog → Posts. The section is hidden while Blog Posts is switched off in Features Activation or nothing is published yet.')
                ->visible(fn (Get $get): bool => self::typeIn($get, [PageSectionType::BlogPosts])),
