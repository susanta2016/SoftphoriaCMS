...
/**
 * Public-facing 🙌 reaction toggle for a Poetry/Prose entry — independent
 * of PoetryProseReviewController's comment submission (client-confirmed,
...
class PoetryProseReactionController extends Controller
{
    public function toggle(Request $request, PoetryProse $poetryProse, ToggleReactionAction $toggle): JsonResponse|RedirectResponse
    {
        ...
        $reacted = $toggle->handle($poetryProse, $user);

        if ($request->wantsJson()) {
            return response()->json([
                'reacted' => $reacted,
                'count' => $poetryProse->reactions()->count(),
            ]);
        }
