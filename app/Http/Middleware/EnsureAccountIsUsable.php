 ...
 * never gated on email verification (see AUTH-003's VerifyEmailAction),
 * only genuinely locked-out statuses are rejected here. Passing this
 * middleware means "authenticated with a usable account," NOT "email
 * verified" — those are two different questions. A future feature that
 * must be verified-only (e.g. commenting on public content) needs its own
 * check against $user->status directly (or a small dedicated middleware
 * at that time); it must not assume EnsureAccountIsUsable/`auth` already
 * covers that for it.
 */
