    ...
    <div class="relative isolate overflow-hidden bg-brand-ivory">
        ...
        <main class="relative mx-auto max-w-md px-4 pt-32 pb-20 sm:px-6 lg:px-8">
            <div class="text-center">
                <h1 class="font-serif text-3xl text-brand-navy sm:text-4xl">Reset Password</h1>
                <div class="my-5 flex items-center justify-center gap-3" aria-hidden="true">
                    <span class="h-px w-12 bg-brand-gold/70"></span>
                    <span class="text-brand-gold">✦</span>
                    <span class="h-px w-12 bg-brand-gold/70"></span>
                </div>
                <p class="text-sm text-brand-navy/70">Choose a new password for your account.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="mt-8 rounded-2xl bg-white p-6 shadow-xl ring-1 ring-brand-navy/5 sm:p-8">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-y-5">
                    <div>
                        ...
                        <input
                            type="email" id="email" name="email" value="{{ old('email', $email) }}" required autofocus autocomplete="email"
