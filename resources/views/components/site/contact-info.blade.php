...
@if ($email || $phone || $whatsapp || $address)
    <div {{ $attributes->class(['space-y-1.5 text-brand-navy/80']) }}>
        @if ($email)
            <p>Email: <a href="mailto:{{ $email }}" class="font-medium text-brand-navy underline decoration-brand-navy/30 underline-offset-2 hover:text-brand-accent">{{ $email }}</a></p>
        @endif
        @if ($phone)
            <p>Phone: <a href="tel:{{ $phone }}" class="font-medium text-brand-navy underline decoration-brand-navy/30 underline-offset-2 hover:text-brand-accent">{{ $phone }}</a></p>
        @endif
        @if ($whatsapp)
            <p>WhatsApp: <a href="https://wa.me/{{ $whatsapp }}" target="_blank" rel="noopener" class="font-medium text-brand-navy underline decoration-brand-navy/30 underline-offset-2 hover:text-brand-accent">Chat on WhatsApp</a></p>
        @endif
        @if ($address)
