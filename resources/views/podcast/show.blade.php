    ...
    >
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            @if ($episode->description)
                <p class="mt-3 max-w-3xl text-sm leading-relaxed text-brand-navy/70">{{ str($episode->description)->stripTags()->pipe(fn ($s) => html_entity_decode($s, ENT_QUOTES | ENT_HTML5))->limit(220) }}</p>
            @endif
            ...
            <div class="mt-4 flex flex-wrap items-center gap-4 text-sm text-brand-navy/60">
                ...
                @endif
                @if (config('features.podcast_comments_enabled') && $reviewCount > 0)
                    <span class="inline-flex items-center gap-1.5">
                ...
                @endif
                @if (config('features.podcast_reactions_enabled') && $reactionCount > 0)
                    <span class="inline-flex items-center gap-1.5">
                    ...
                    <div class="border-b border-brand-navy/10 pb-10">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @if (config('features.podcast_reactions_enabled'))
                                    @auth
                        ...
                        {{-- Internal comments disabled for Podcast (client-
                            confirmed, 2026-09-04) — discussion happens on
                            YouTube instead. config('features.podcast_comments_enabled'),
                            enforced server-side too in PodcastEpisodeReviewController. --}}
                        @if (config('features.podcast_comments_enabled'))
                            @if ($reviews->isEmpty())
