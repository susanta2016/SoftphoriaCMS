 ...
 * artwork, publish date, season/episode, embed URL and SEO metadata, per
 * docs/Reference UI/Frontend's approved Podcast List/Episode designs.
 * Streaming links are no longer edited here (f2d855e); stored ones must
 * survive a save.
 */
class PodcastEpisodeTest extends TestCase
{
    ...
    public function test_admin_can_create_an_episode_with_seo_and_topics(): void
    {
    ...
    /**
     * The Streaming Link field was removed from the admin form (f2d855e) but
     * podcast_links rows were deliberately kept — saving must not touch them.
     */
    public function test_saving_an_episode_leaves_its_stored_streaming_links_untouched(): void
    {
        ...
        Livewire::actingAs($this->admin())
            ...
            ->assertDontSee('Streaming Link')
            ->fillForm(['title' => 'Renamed Episode'])
            ->call('save')
            ->assertHasNoFormErrors();

        $episode->refresh();
        $this->assertSame('Renamed Episode', $episode->title);
        $this->assertCount(1, $episode->links);
        $this->assertSame($link->id, $episode->links->first()->id);
        $this->assertSame('https://open.spotify.com/episode/original', $episode->links->first()->url);
    }
